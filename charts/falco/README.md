...
| falco.plugins_hostinfo | bool | `true` | Control host info support for plugins. `proc-fs` is rendered when `driver.enabled=true` or `falco.plugins_hostinfo=true`; therefore, setting `driver.enabled=false` and `falco.plugins_hostinfo=false` drops the `proc-fs` hostPath requirement for plugin-only deployments. |
